from typing import Type, Generic, Any, Self
...
class SQLAlchemyModelPaginator(Generic[PaginationItem]):
    ...
    async def get_list(
        ...
        async with self.session as s:
            ...
            items = (await s.execute(statement_items)).scalars().all()
        ...
        return PaginatedResponseSchema(
            ...
            total_items=count,
            items=[
                self.pagination_item_type.model_validate(item, from_attributes=True)
                for item in items
            ],
        )
